...
export async function getVocabFromBackend() {
    let fetchUrl = 'https://as-vocab-backend.herokuapp.com/qb-vocab-table'
    ...
    .then((res) => {
        if(res.ok){
            return res.json().then((res) => {
                ...
                console.log(json);
                printFields(json);
                const linkedFieldsToNumsArr = createFieldsJSON(qb.vocabulary.fields, json.fields)
                const tableArr = createTable2(json.data, linkedFieldsToNumsArr)
                return tableArr
            }) 
        }
...
export async function getExamplesFromBackend() {
    let fetchUrl = 'https://as-vocab-backend.herokuapp.com/qb-example-table'
...
export async function getLessonsFromBackend() {
    let fetchUrl = 'https://as-vocab-backend.herokuapp.com/qb-lesson-table'
